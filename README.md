...
- `kind` は `"講演"` `"MC"` `"パネル"` `"研修"` `"取材"` `"出演"` から選びます（絞り込みチップは使われている種類だけ自動で出ます）
...
`data.js` のいちばん上の `SAMPLE_MODE` は、現在 `false`（実データ運用）になっています。
仮の内容を入れて確認したいときだけ `true` にすると、黄色い「※ 表示中のデータはサンプルです」の帯が出ます。

### トップページの文言（HERO / STATS / PROFILE_TEXT / BAND / KODAWARI_TITLE）

これらもすべて `data.js` にあります。HTMLは触らなくて大丈夫です。

| 定数 | 変わる場所 |
|---|---|
| `HERO` | いちばん上の「講演・イベント出演のご依頼を承ります」、肩書き3行、リード文、2つのボタン |
| `STATS` | その下の4つの数字。`value: "auto:lectures"` と書いた項目は登壇件数が自動で入ります |
...
`TOPICS` の各テーマにある `short` は、トップページ最上部の小さなチップ（丸いボタン）に使う短い名前です。
長い正式タイトルは `title` に、チップ用の短い名前は `short` に書いてください。
...
| ページ | 中身 |
|---|---|
| （ナビ） | ナビメニュー（全ページ共通）は `app.js` の `NAV_ITEMS`：トップ→プロフィール→講演実績→SNS・動画→ブログ→ギャラリー。「こだわり」は2026-09-08にメニューから外した（セクション自体はトップに残る） |
...
```js
{
  ...
  record:  { ja: "どこで実施したか（空なら「実績」バッジは出ません）" },
  tags:    { ja: ["タグ1", "タグ2"] }
},
```
